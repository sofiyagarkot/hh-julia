...
function sigmoid(x, slope, x_offset, y_offset)
    return 1 / (1 + exp(-slope * (x - x_offset))) + y_offset
end

function logit(y, slope, x_offset, y_offset)
    """
...
function define_problem_with_link_function(duration = 50.0)
    αm(V, VT) = -0.32 * (V - VT - 13) / (exp(-(V - VT - 13) / 4) - 1)
    βm(V, VT) = 0.28 * (V - VT - 40) / (exp((V - VT - 40) / 5) - 1)
    ...
    function f(du, u, params, t)
        ...
        m_sigmoid = sigmoid(m, slope, x_offset, y_offset)
        n_sigmoid = sigmoid(n, slope, x_offset, y_offset)
        h_sigmoid = sigmoid(h, slope, x_offset, y_offset)

        INa = gNa * m_sigmoid^3 * n_sigmoid * (exp(V) - ENa) * area
        IK = gK * h_sigmoid^4 * (exp(V) - EK) * area
        Ileak = gleak * (exp(V) - Eleak) * area
        Cm = C * area
        du[1] = dVdt =(1/exp(V)) * (-(Ileak + INa + IK - I_inj) / Cm)
    end
